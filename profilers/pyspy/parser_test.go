...
`
	...
	if got.Threads[1].ID != "1001" {
		t.Fatalf(
			"second thread ID = %q, want %q",
			got.Threads[1].ID,
			"1001",
		)
	}

	if got.Threads[1].State != "idle" {
		t.Fatalf(
			"second thread state = %q, want %q",
			got.Threads[1].State,
			"idle",
		)
	}
...
func TestParseDumpFrame(
	...
	tests := []struct {
		...
		{
			name: "frame without line number",
...
func TestParseProfileDataChromeTrace(
	t *testing.T,
) {
	const output = `{"traceEvents":[]}`
...
func TestParseProfileDataRawReturnsJSONString(
	t *testing.T,
) {
	const output = "main;train;forward 10\n"
...
func TestParseProfileDataRejectsUnsupportedFormat(
	t *testing.T,
) {
	_, err := parseProfileData(
		coreprofiler.Request{
